...
Config::Config(std::string jsonfilename)
  ...
  //SergioL: To enable control symbol, check. control_data_size < (codeblocks_per_sym * bytes_per_code_block)
...
BroadcastControlData Config::DecodeBroadcastSlots(const int16_t* const bcast_iq_samps) {
  ...
  //SergioL: Decodebroadcast process
  return (reinterpret_cast<BroadcastControlData*>(decode_buff.data()))[0];
}
...
void Config::GenBroadcastSlots(
  ...
  for (size_t i = 0; i < this->frame_.NumDlControlSyms(); i++) {
    std::memcpy(bcast_bits_buffer.data(), &ctrl_msg, sizeof(ctrl_msg));
